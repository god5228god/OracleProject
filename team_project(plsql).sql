...
--○ 프로시저명: PRC_GR_C_PF
-- GRADE 테이블에 INSERT - 교수자만 가능 (로그인아이디)
-- 교수자가 본인 과목에 한해서만 생성가능, 점수는 업데이트로만 가능
-- 중도탈락 학생 제외
...
BEGIN
    ...
    IF (V_LOGIN_ID = 'PF') THEN
        ...
        -- 교수아이디가 개설한 과목이 없거나 그 개설과목들 모두에 수강생이 한 명도 없는 경우 예외
        SELECT COUNT(*) INTO V_CNT
        FROM (SELECT OS.OS_CODE, ER.ER_CODE, OS.OS_EDATE
    ...
    EXCEPTION
        ...
        WHEN USER_DEFINE_ERROR2 THEN
            RAISE_APPLICATION_ERROR(-20020, '생성할 수 있는 성적이 없습니다.');
END;



--○ 프로시저명: PRC_GR_C_AD
-- GRADE 테이블에 INSERT - 관리자만 가능 (로그인아이디, 수강신청코드, 개설과목코드, 출결점수, 필기점수, 실기점수)
...
BEGIN
    ...
    EXCEPTION
        ...
        WHEN USER_DEFINE_ERROR3 THEN
            RAISE_APPLICATION_ERROR(-20001, '중복된 데이터입니다.');
...
BEGIN
    ...
    EXCEPTION
        WHEN USER_DEFINE_ERROR1 THEN
            RAISE_APPLICATION_ERROR(-20009,'권한이 없습니다.');
        WHEN USER_DEFINE_ERROR2 THEN
            RAISE_APPLICATION_ERROR(-20014,'삭제할 수 있는 성적이 없습니다.');
END;
