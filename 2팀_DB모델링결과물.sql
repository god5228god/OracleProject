...
-------------------------------------------------------------------------
--★ 9. 뷰 생성
-- 3-1. 관리자용 - 교수자 배정 현황 뷰
CREATE OR REPLACE VIEW VIEW_ADMIN_PROF_LIST 
SELECT
     P.PF_ID
    ,P.PF_NAME AS 교수자명
    ,S.SUB_NAME AS 과목명
...
-- 3-2. 관리자용 - 과정 상세 정보 뷰
CREATE OR REPLACE VIEW VIEW_ADMIN_COURSE_LIST 
SELECT
     CR.CR_CODE
    ,CR.CR_NAME AS 과정명
    ,CL.CL_NAME AS 강의실
...
-- 3-3. 관리자용 - 과목 상세 정보 뷰
CREATE OR REPLACE VIEW VIEW_ADMIN_SUBJECT_LIST 
SELECT
     OS.OS_CODE
    ,CR.CR_NAME AS 과정명
    ,CL.CL_NAME AS 강의실
...
-- 3-4. 관리자용 - 학생 목록 및 중도탈락 상태 뷰
CREATE OR REPLACE VIEW VIEW_ADMIN_STUDENT_LIST 
SELECT
     ST.ST_ID
    ,ST.ST_NAME AS 학생이름
    ,CR.CR_NAME AS 과정명
...
-- 3-5. 교수자용 - 강의 과목 목록 뷰
CREATE OR REPLACE VIEW VIEW_PROF_SUBJECT_LIST 
SELECT
     OS.PF_ID
    ,OS.OS_CODE
    ,S.SUB_NAME AS 과목명
...
-- 3-6. 교수자용 - 성적 입력 대상 학생 목록 뷰
CREATE OR REPLACE VIEW VIEW_PROF_SCORE_INPUT 
SELECT
     OS.PF_ID
    ,OS.OS_CODE
    ,S.SUB_NAME AS 과목명
...
-- 3-7. 교수자용 - 과목별 성적 및 등수 출력 뷰
CREATE OR REPLACE VIEW VIEW_PROF_SCORE_OUTPUT 
SELECT
     OS.PF_ID
    ,OS.OS_CODE
    ,S.SUB_NAME AS 과목명
...
-- 3-8. 학생용 - 수강 과목 목록 뷰
CREATE OR REPLACE VIEW VIEW_STUDENT_SUBJECT_LIST 
SELECT
     ST.ST_ID
    ,OS.OS_CODE
    ,S.SUB_NAME AS 과목명
...
-- 3-9. 학생용 - 성적 조회 뷰 (통합)
CREATE OR REPLACE VIEW VIEW_STUDENT_SCORE 
SELECT
     ST.ST_ID
    ,ST.ST_NAME AS 학생이름
    ,CR.CR_NAME AS 과정명
