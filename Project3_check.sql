...
BEGIN  
    -- 개설과정 존재 및 종료일 확인
    SELECT COUNT(*), MAX(OC_EDATE) 
    INTO V_OC_CNT, V_OC_EDATE
    FROM OPEN_COURSE
    WHERE OC_CODE = P_OC_CODE;
    ...
    -- 종료된 과정 확인
    IF V_OC_EDATE < SYSDATE THEN
        RAISE_APPLICATION_ERROR(-20024, '종료된 과정에는 수강신청할 수 없습니다.');
    END IF;
